...
from ..models import TeacherHours, Paychecks
...
@router.get("/all_paychecks/", status_code=status.HTTP_200_OK, response_model=List[PaycheckResponse])
async def get_all_paychecks_for_teacher(db:db_dependancy,is_payed:bool=None,start_date:date=None,end_date:date=None, teacher_id: int =None,
    page: int = Query(gt=0),
    limit: int = Query(gt=0, default=20)):
    ...
    return await crud.get_all_paychecks(db,page,limit,teacher_id,is_payed,start_date,end_date)
...
@router.put("/pay", status_code=status.HTTP_201_CREATED, response_model=PaycheckResponse)
